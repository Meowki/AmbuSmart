...
from schemas.case_history import CaseHistory, CaseHistoryCreate
...
def get_all_case_histories(db: Session = Depends(get_db)):
    ...
    return case_history if isinstance(case_history,list) else [case_history]

@router.get("/{patient_id}", response_model=list[CaseHistory])
